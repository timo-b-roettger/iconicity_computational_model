...
Signal/failure drift: on incorrect trials, the stored signal is updated by moving it a small fixed step toward the iconic prototype for that referent type. In addition to this deterministic shift, small Gaussian noise is added. Thus, signal drift on failure is directional toward the prototype (iconicity) but includes random variation from noise. The failure drift could be interpreted as an articulatory bias, i.e., an intentional iconicity parameter on behalf of the speaker when noticing that communication fails?
...
d.param_grid <- expand.grid(
  ...
  failure_step = seq(0, 0.06, length.out = 6),
...
compute_iconicity <- function(d.history, n_rounds) {
  ...
  mean(d.iconicity.grid$iconicity[d.iconicity.grid$bins >= final_bins])
}
...
for (i in 1:nrow(d.param_grid)) {
  ...
  hist <- run_interaction_sim(
    data = empty_df,
    n_sim = 4,            # small number for medium-fast grid search
    n_referents = 6,
    n_rounds = 600,       # enough time for iconicity to emerge
    drift_sd = 0.05,
    #learn_rate = 0.005,
  ...
  # compute iconicity
  mean_iconicity <- compute_iconicity(hist, n_rounds = 400)
...
#Read in model file
d.grid_results.old <- readRDS("iconicity-model_files/grid-search.rds")
...
p.learned.iconicity.sep.memories + p.produced.iconicity.sep.memories
```
