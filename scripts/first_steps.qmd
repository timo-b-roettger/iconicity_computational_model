...
drift_sd <- 0.1
learn_rate <- 0.005
iconicity_boost <- 2 # multiplicator for distance to iconic prototypes
...
## each agent has initial probability ~ 0.5 ± noise (based on beta[2,2])
...
guess_probability <- function(agent_guess, signal_xy, ref_type) {
  ...
  ## ease is reduced by a factor of iconicity_boost which represents the strength of iconicity affecting guessing
...
ggplot(iconicity, aes(x = bins, 
  ...
  labs(title = "Iconicity Over Time", 
       y = "Iconicity (low = iconic, high = anti-iconic)", 
       x = "Rounds in 100s") +
  #scale_y_continuous(limits = c(0,1)) + 
  theme_minimal()
