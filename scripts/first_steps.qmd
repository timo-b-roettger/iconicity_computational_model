...
n_referents <- 10 # number of unique referents in guessing game
n_small <- 5 # number of small referents
n_large <- 5 # number of large referents
n_rounds <- 2000 # number of interaction rounds
drift_sd <- 0.1 # amount of variation introduced during production
learn_rate <- 0.005 # amount of added memory strengthening for words per round
iconicity_boost <- 1 # multiplicator for distance to iconic prototypes
...
signal_ease <- function(sig, type) {
  ...
  ease <- exp(-2 * dist)   # the -2 value controls the sharpness of the drop with distance
  ease
}

# explore result of this function
# grid <- expand.grid(
...
guess_probability <- function(agent_guess, signal_xy, ref_type) {
  ...
  ## the added learning boost of ease is reduced by a factor of iconicity_boost which represents the strength of iconicity affecting guessing
...
for (t in 1:n_rounds) {
  ...
  # history$ease[t] <- as.numeric(ease)
  history <- rbind(
      history,
      data.frame(
        sim      = n,
